"""After-hours watchlist builder (4:00–8:00 PM ET).

Builds a watchlist of candidates for the next trading session.
Does not trade directly — thin liquidity in after-hours.
"""
...
from utils.time_utils import is_afterhours, now_et

logger = logging.getLogger("trading_bot.afterhours")


class AfterhoursScanner:
    """After-hours watchlist builder."""
    ...
    async def scan_once(self) -> list[dict]:
        ...
        logger.info(f"After-hours watchlist: {len(candidates)} candidates")
        return self._watchlist

    async def start(self) -> None:
        """Run after-hours scanning loop."""
        self._running = True
        logger.info("After-hours scanner started")
        while self._running and is_afterhours():
            await self.scan_once()
            await asyncio.sleep(WATCHLIST_REFRESH_SECONDS)
        logger.info("After-hours session ended")

    def stop(self) -> None:
        self._running = False

    async def _enrich_with_float(self, symbol: str) -> dict:
        """Supplement with float data from yfinance."""
        try:
