"""Fibonacci confirmation strategy — 4-state machine on 1-min bars.

Per-symbol state machines that persist across 30-second scan cycles:
  SCANNING → CANDLE_1_CONFIRMED → READY_TO_BUY → IN_POSITION

Uses 1-min IBKR bars, dual-series recursive Fibonacci levels,
and SMA 20 + SMA 200 gate for entry confirmation.
"""
...
from config.settings import (
    FIB_CONFIRM_BAR_SIZE,
    FIB_CONFIRM_BAR_DURATION,
    FIB_CONFIRM_STOP_PCT,
    FIB_CONFIRM_TARGET_LEVELS,
    FIB_CONFIRM_MAX_ENTRIES_PER_DAY,
    FIB_CONFIRM_ENTRY_WINDOW_END,
    FIB_CONFIRM_WARMUP_BARS,
    FIB_CONFIRM_SMA_SHORT,
    FIB_CONFIRM_SMA_LONG,
    FIB_LOOKBACK_YEARS,
    SCAN_PRICE_MIN,
    SCAN_PRICE_MAX,
)
from scanner.strength_scanner import StrengthSignal
...
from strategies.indicators import bars_to_dataframe, is_above_sma
from utils.time_utils import now_et, today_str

logger = logging.getLogger("trading_bot.fib_confirm_strategy")


class SymbolPhase(Enum):
    SCANNING = "SCANNING"
    CANDLE_1_CONFIRMED = "CANDLE_1_CONFIRMED"
    READY_TO_BUY = "READY_TO_BUY"
    IN_POSITION = "IN_POSITION"


@dataclass
class ConfirmEntryRequest:
...
class SymbolState:
    ...
    # CANDLE_1_CONFIRMED state
    candle1_fib_idx: int = -1
    candle1_low: float = 0.0
    # Tracking
...
class FibConfirmStrategy:
    """Per-symbol 4-state machines for fib confirmation entries on strong stocks."""
    ...
    def __init__(self) -> None:
        ...
        h, m = FIB_CONFIRM_ENTRY_WINDOW_END.split(":")
    ...
    def mark_position_closed(self, symbol: str) -> None:
        ...
        if st:
            ...
            st.candle1_fib_idx = -1
            st.candle1_low = 0.0
            logger.info(f"[{symbol}] -> SCANNING (position closed)")

    async def process_cycle(
        self, strength_signals: list[StrengthSignal]
    ) -> list[ConfirmEntryRequest]:
        """Run one strategy cycle across all tracked symbols.

        Returns ConfirmEntryRequest list for the entry executor to act on.
        """
        self._reset_daily_if_needed()
        requests: list[ConfirmEntryRequest] = []
        ...
        if self._entries_today >= FIB_CONFIRM_MAX_ENTRIES_PER_DAY:
            return []
        ...
        for signal in strength_signals:
            try:
                ...
                if req:
                    requests.append(req)
                    if self._entries_today + len(requests) >= FIB_CONFIRM_MAX_ENTRIES_PER_DAY:
                        break
    ...
    async def _process_symbol(
        self, signal: StrengthSignal
    ) -> Optional[ConfirmEntryRequest]:
        """Process one symbol through the 4-state machine."""
        ...
        bars = await self._conn.get_historical_data(
            st.contract,
            duration=FIB_CONFIRM_BAR_DURATION,
            bar_size=FIB_CONFIRM_BAR_SIZE,
            what_to_show="TRADES",
            use_rth=False,
        )
        df = bars_to_dataframe(bars)
        if len(df) < FIB_CONFIRM_WARMUP_BARS + 2:
            return None

        # Only process NEW bars since last cycle
        new_bar_count = len(df)
        start_idx = max(FIB_CONFIRM_WARMUP_BARS, st.last_bar_count)
        if new_bar_count <= st.last_bar_count:
            # No new bars — still check CANDLE_1_CONFIRMED on last bar
            if st.phase == SymbolPhase.CANDLE_1_CONFIRMED and new_bar_count > 0:
                start_idx = new_bar_count - 1
            else:
                return None
        st.last_bar_count = new_bar_count

        # Update gap high from warmup period
        warmup_high = float(df["high"].iloc[:FIB_CONFIRM_WARMUP_BARS].max())
        ...
        for i in range(start_idx, new_bar_count):
            ...
            if st.phase == SymbolPhase.SCANNING:
                # Look for candle that touches a fib level and closes above it,
                # with SMA 20 + SMA 200 gate
                for fi, fp in enumerate(st.fib_prices):
                    if bar_low <= fp and bar_close > fp:
                        # SMA gate: close must be above both SMA 20 and SMA 200
                        df_up_to_bar = df.iloc[:i + 1]
                        above_sma_short = is_above_sma(
                            df_up_to_bar, FIB_CONFIRM_SMA_SHORT
                        )
                        above_sma_long = is_above_sma(
                            df_up_to_bar, FIB_CONFIRM_SMA_LONG
                        )

                        if above_sma_short and above_sma_long:
                            st.phase = SymbolPhase.CANDLE_1_CONFIRMED
                            st.candle1_fib_idx = fi
                            st.candle1_low = bar_low
                            logger.info(
                                f"[{symbol}] SCANNING -> CANDLE_1_CONFIRMED: "
                                f"bar touched fib ${fp:.4f} (idx={fi}), "
                                f"SMA20+200 pass"
                            )
                            break

            elif st.phase == SymbolPhase.CANDLE_1_CONFIRMED:
                # Confirmation candle: must not make a lower low
                if bar_low >= st.candle1_low:
                    # CONFIRMED — transition to READY_TO_BUY and generate entry
                    fib_level = st.fib_prices[st.candle1_fib_idx]
                    stop_price = round(fib_level * (1 - FIB_CONFIRM_STOP_PCT), 4)

                    target_idx = st.candle1_fib_idx + FIB_CONFIRM_TARGET_LEVELS
                    ...
                    logger.info(
                        f"[{symbol}] CANDLE_1_CONFIRMED -> READY_TO_BUY: "
                        f"fib=${fib_level:.4f}, stop=${stop_price:.4f}, "
                        f"target=${target_price:.4f}"
                    )

                    # Transition to READY_TO_BUY (entry executor will move to IN_POSITION)
                    st.phase = SymbolPhase.READY_TO_BUY

                    return ConfirmEntryRequest(
                        symbol=symbol,
                        contract=st.contract,
                        fib_level=fib_level,
                        fib_idx=st.candle1_fib_idx,
                    ...
                    # Failed — lower low made, back to scanning
                    logger.info(
                        f"[{symbol}] CANDLE_1_CONFIRMED -> SCANNING (lower low "
                        f"${bar_low:.4f} < ${st.candle1_low:.4f})"
                    )
                    st.phase = SymbolPhase.SCANNING
                    st.candle1_fib_idx = -1
                    st.candle1_low = 0.0
    ...
    async def _init_symbol_state(
        self, signal: StrengthSignal, today: str
        ...
        logger.info(
            ...
            f"{len(fib_prices)} fib levels, gain +{signal.gain_pct:.1f}%, "
