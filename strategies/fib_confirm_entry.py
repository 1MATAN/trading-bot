"""Entry execution for fibonacci confirmation strategy.

Takes a ConfirmEntryRequest from the strategy, performs risk checks,
...
from strategies.fib_confirm_strategy import ConfirmEntryRequest, FibConfirmStrategy
from utils.time_utils import now_et

logger = logging.getLogger("trading_bot.fib_confirm_entry")


class FibConfirmEntry:
    """Execute entries from fib confirmation strategy signals."""
    ...
    async def execute_entry(self, request: ConfirmEntryRequest) -> bool:
        """Execute a single entry from a ConfirmEntryRequest.
        ...
        # 2. Already have a position in this symbol?
        ...
        self._trade_logger.log_entry(
            ...
            stop_price=request.stop_price,
            notes=(
                f"fib_confirm gain={request.gain_pct:+.1f}% "
        ...
        logger.info(
            ...
            f"target=${request.target_price:.4f} | "
            f"gain={request.gain_pct:+.1f}% | oca={oca_group}"
        )
